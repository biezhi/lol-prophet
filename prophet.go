...
func (p *Prophet) initGin() {
	...
	engine.NoRoute(func(c *gin.Context) {
		...
		if flag {
			content, err := ioutil.ReadFile("web/dist/index.html")
			if (err) != nil {
...
func (p *Prophet) initWebview() {
	clientCfg := global.GetClientConf()
	defaultUrl := "http://localhost:4396/"
...
func (p Prophet) CalcEnemyTeamScore() {
	...
	selfIDListScore := []global.UserScore{}
	enemyIDListScore := []global.UserScore{}
	blackIDListScore := []global.BlockUserScore{}
	...
	for _, sID := range selfTeamUsers {
		...
		g.Go(func() error {
			...
			mu.Lock()
			selfIDListScore = append(selfIDListScore, *sScore)
			mu.Unlock()
			return nil
		})
	}
	for _, eID := range enemyTeamUsers {
		eID := eID
		g.Go(func() error {
			eScore, err := GetSummonerInfo(eID)
			if err != nil {
				return nil
			}
			mu.Lock()
			enemyIDListScore = append(enemyIDListScore, *eScore)
			mu.Unlock()
			return nil
		})
	}
	allSummonerIDList := enemyTeamUsers
	allSummonerIDList = append(allSummonerIDList, selfTeamUsers...)

	var items []global.BlacklistItem
	global.SqliteDB.Where("summoner_id IN ?", allSummonerIDList).Find(&items)
	if len(items) > 0 {
		for _, item := range items {
			blackIDListScore = append(blackIDListScore, global.BlockUserScore{
	...
	// 在表中显示是否拉黑
	_ = g.Wait()
	ws.BroadcastMsg(ws.Msg{
		Type: "summonerMap",
		Data: global.SummonerIDMapScore{
			SelfTeam:   selfIDListScore,
			EnemyTeam:  enemyIDListScore,
			BlackUsers: blackIDListScore,
		},
	})
	global.SetSummonerIDList(global.SummonerIDMapScore{
		SelfTeam:   selfIDListScore,
		EnemyTeam:  enemyIDListScore,
		BlackUsers: blackIDListScore,
	})
